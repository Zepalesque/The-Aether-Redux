...
public abstract class AbstractBookshelfSet<B extends BookshelfBlock> extends LogWallWoodSet {
	...
	public void langData(ReduxLanguageProvider data) {
		...
		data.addLore(
			this.bookshelf(),
			"A nice bookshelf made of "
				+ name
				+ " wood. These are nice for decoration, and also will enhance the abilities of Enchanting Tables!"
		);
	}
	...
	public void lootData(ReduxBlockLootProvider data) {
		super.lootData(data);
		data.add(this.bookshelf().get(), bookshelf ->
			data.createSingleItemTableWithSilkTouch(bookshelf, Items.BOOK, ConstantValue.exactly(3.0F))
		);
	}
