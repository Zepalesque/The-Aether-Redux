	...
	public void performBonemeal(ServerLevel lvl, RandomSource rand, BlockPos pos, BlockState state) {
		...
		loop: for (var i = 0; i < 128; ++i) {
			...
			for (var j = 0; j < i / 16; ++j) {
				...
				if (
					!lvl.getBlockState(testPos.below()).is(this) ||
					lvl.getBlockState(testPos).isCollisionShapeFullBlock(lvl, testPos)
				) continue loop;
			}

			var testState = lvl.getBlockState(testPos);
			if (testState.is(grass) && rand.nextInt(10) == 0) ((BonemealableBlock) grass).performBonemeal(
				lvl,
				rand,
				testPos,
				testState
			...
			// i hope adding an else here was ok
				...
				if (rand.nextInt(8) == 0) {
					...
					featureHolder = (
						(RandomPatchConfiguration) list.get(rand.nextInt(list.size())).config()
					).feature();
					featureHolder.value().place(lvl, lvl.getChunkSource().getGenerator(), rand, testPos);
				} else if (lvl.ensureCanWrite(testPos)) // TODO
					//lvl.setBlock(
					//	testPos,
					//	rand.nextFloat() < 0.6F
