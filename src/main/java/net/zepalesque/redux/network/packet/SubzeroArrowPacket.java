...
public record SubzeroArrowPacket(int entityID, boolean isSubzero) implements BasePacket {
    ...
    public void execute(Player playerEntity) {
        if (Minecraft.getInstance().player != null && Minecraft.getInstance().level != null && Minecraft.getInstance().player.getLevel().getEntity(this.entityID) instanceof AbstractArrow arrow) {
