...
public class AdrenalineModule implements PlayerTickModule {
    ...
    public void tick() {
        if (this.player.getLevel().isClientSide()) {
            ...
            if (this.adrenalineStrength > 0) {
                this.maxPulseTicks = BackportUtil.lerpInt((float) this.adrenalineStrength, 20, 10);
            } else {
    ...
    protected void doPulse(double strength) {
        this.player.getLevel().playSound(this.player, this.player.getX(), this.player.getY(), this.player.getZ(), getHeartbeat(strength), SoundSource.PLAYERS, 0.8F, 1F);
    }
