...
public class CockatriceExtensionCapability implements CockatriceExtension {
    ...
    public void handleBurning() {
        if (!this.cockatrice.getLevel().isClientSide() && this.cockatrice.isAlive()) {
    ...
    public void handleTargetAnim() {
        if (ReduxConfig.COMMON.improved_cockatrice_behavior.get()) {
            if (this.getCockatrice().getLevel().isClientSide()) {
    ...
    public void refreshNearby() {
        if (this.cockatrice.getTarget() != null) {
            Level level = this.cockatrice.getLevel();
    ...
    protected boolean isSunBurnTick() {
        if (this.cockatrice.getLevel().isDay() && !this.cockatrice.getLevel().isClientSide) {
            ...
            if (f > 0.5F && this.cockatrice.getLevel().random.nextFloat() * 30.0F < (f - 0.4F) * 2.0F && !flag && this.cockatrice.getLevel().canSeeSky(blockpos)) {
                return true;
            }
        }
