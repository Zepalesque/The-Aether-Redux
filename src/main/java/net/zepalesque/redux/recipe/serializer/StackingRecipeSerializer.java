...
public class StackingRecipeSerializer<T extends StackingRecipe> implements RecipeSerializer<T> {
    ...
    public T fromJson(ResourceLocation id, JsonObject json) {
        ...
        if (json.has("sound")) {
            ...
            sound = Registry.SOUND_EVENT.getOptional(new ResourceLocation(soundObject.getAsString()));
        } else {
    ...
    public T fromNetwork(ResourceLocation id, FriendlyByteBuf buffer) {
        ...
        ItemStackConstructor result = buffer.readWithCodec(ItemStackConstructor.CODEC);
        Optional<CompoundTag> additional = buffer.readOptional(FriendlyByteBuf::readNbt);
        Optional<SoundEvent> sound = buffer.readOptional(FriendlyByteBuf::readResourceLocation).flatMap(Registry.SOUND_EVENT::getOptional);
        return this.factory.create(id, ingredient, result, additional,sound);
    }
    ...
    public void toNetwork(FriendlyByteBuf buffer, T recipe) {
        ...
        buffer.writeWithCodec(ItemStackConstructor.CODEC, recipe.getResult());
        buffer.writeOptional(recipe.getAdditionalData(), FriendlyByteBuf::writeNbt);
        buffer.writeOptional(recipe.getSound().map(Registry.SOUND_EVENT::getKey), FriendlyByteBuf::writeResourceLocation);
    }
