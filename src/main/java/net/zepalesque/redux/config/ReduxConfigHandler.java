...
public class ReduxConfigHandler {
	public static void setup(ModContainer mod, IEventBus bus) {
		try {
			...
			mod.registerConfig(
				...
				ReduxConfig.CLIENT_SPEC,
				Redux.MODID + "/client.toml"
			);
			mod.registerConfig(
				ModConfig.Type.COMMON,
				ReduxConfig.COMMON_SPEC,
				Redux.MODID + "/common.toml"
			);
			mod.registerConfig(
				ModConfig.Type.SERVER,
				ReduxConfig.SERVER_SPEC,
				Redux.MODID + "/server.toml"
			);
		} catch (Exception e) {
	...
	/*private static void createAndLoadConfigs(
		ModContainer mod,
		ModConfig.Type type,
		ModConfigSpec spec,
		String path
