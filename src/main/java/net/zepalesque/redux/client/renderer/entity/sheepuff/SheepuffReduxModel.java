...
public class SheepuffReduxModel extends QuadrupedModel<Sheepuff> {
	...
	public static LayerDefinition createBodyLayer() {
		...
		var head = partdefinition.addOrReplaceChild("head", CubeListBuilder.create().texOffs(20, 1).mirror().addBox(-5.0F, -4.0F, -5.0F, 2.0F, 4.0F, 3.0F, new CubeDeformation(0.0F)).mirror(false)
		...
		var wisp_left = head.addOrReplaceChild("wisp_left", CubeListBuilder.create(), PartPose.offset(5.0F, -3.0F, -2.0F));
		
		var bend_left = wisp_left.addOrReplaceChild("bend_left", CubeListBuilder.create().texOffs(54, -5).addBox(0.0F, -2.0F, 0.0F, 0.0F, 4.0F, 5.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(0.0F, 0.0F, 0.0F, 0.0F, 0.4363F, 0.0F));
		...
		var wisp_right = head.addOrReplaceChild("wisp_right", CubeListBuilder.create(), PartPose.offset(-5.0F, -3.0F, -2.0F));
		
		var bend_right = wisp_right.addOrReplaceChild("bend_right", CubeListBuilder.create().texOffs(54, -5).mirror().addBox(0.0F, -2.0F, 0.0F, 0.0F, 4.0F, 5.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offsetAndRotation(0.0F, 0.0F, 0.0F, 0.0F, -0.4363F, 0.0F));
	...
	public void setupAnim(Sheepuff sheepuff, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		var ticker = sheepuff.level().tickRateManager();
		
		var partialTicks = mc.getTimer().getGameTimeDeltaPartialTick(!ticker.isEntityFrozen(sheepuff));
		...
		var hangSwing = Mth.cos(limbSwing * 0.6662F * 0.5F + (float) Math.PI) * 0.0625F * limbSwingAmount;
		var hangWave = LegacyAnimUtil.breathe(sheepuff, partialTicks, 0.5F + 3.5F * limbSwingAmount, 1F, (float) (Math.PI * 0.75));
		var wave1 = LegacyAnimUtil.breathe(sheepuff, partialTicks, 0.4F + 2.6F * limbSwingAmount, 2F, 0F);
		var wave2 = LegacyAnimUtil.breathe(sheepuff, partialTicks, 0.4F + 2.6F * limbSwingAmount, 2F, (float) (Math.PI * 0.25));
