	...
	public static void registerTintHandlers(RegisterColorHandlersEvent.Block event) {
		event.register(
			(state, level, pos, tintIndex) -> {
				...
				final var sampleScale = 35d;
				// TODO: null check level and pos which by the signature of BlockColor#getColor may be null
				var samplePos = pos == null ? BlockPos.ZERO : pos;
				var noise = PRISMA_NOISE.getValue(samplePos.getX() / posScale, samplePos.getY() / posScale, samplePos.getZ() / posScale) * sampleScale;
