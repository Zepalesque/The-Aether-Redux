	...
	public boolean place(FeaturePlaceContext<DebugNoiseFeature.Config> context) {
		...
		for (var x = 0; x < 16; x++) {
			for (var z = 0; z < 16; z++) {
				...
				var initCalc = noiseFun.compute(
					new DensityFunction.SinglePointContext(xCoord, cfg.sampleY(), zCoord)
				);
				var inverped = Mth.clamp(Mth.inverseLerp(initCalc, -1, 1), 0d, 1d);
	...
	public record Config(
		...
		public static final Codec<DebugNoiseFeature.Config> CODEC = RecordCodecBuilder.create(
			(builder) ->
				builder
					.group(
						BlockStateProvider.CODEC.listOf()
							.fieldOf("gradient")
							.forGetter(DebugNoiseFeature.Config::gradient),
						...
						DensityFunction.HOLDER_HELPER_CODEC.fieldOf("noise").forGetter(
							DebugNoiseFeature.Config::noise
						) // lack of trailing commas my beloathed
					)
