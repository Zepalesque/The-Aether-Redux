...
public class JellyshroomFeature extends Feature<JellyshroomFeature.Config> {
	public JellyshroomFeature(Codec<Config> codec) {
		super(codec);
	}
	...
	public boolean place(FeaturePlaceContext<Config> context) {
			...
			for (int x = -radius; x <= radius; x++) {
				for (int z = -radius; z <= radius; z++) {
					...
					if ((center && middlecorners) || isDiamond) {
						...
						toPlace.putIfAbsent(
							immutable2,
							context.config().jelly.getState(context.random(), immutable2)
						);
					}
				}
			}
		...
		if (height >= 7) {
			...
			Direction d2 = Util.getRandom(
				Direction.Plane.HORIZONTAL.stream()
					.filter(direction -> direction != d1)
					.toList(),
				context.random()
			);
			int minAbove = 1;
			...
			toPlace.putIfAbsent(
				immutable3,
				context.config().jelly.getState(context.random(), immutable3)
			);
			...
			toPlace.putIfAbsent(
				immutable4,
				context.config().jelly.getState(context.random(), immutable4)
			);
		}
