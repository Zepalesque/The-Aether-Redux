...
public class TestBlockBelowPlacementPatch extends Feature<TestBelowPlaceConfig> {
   public TestBlockBelowPlacementPatch(Codec<TestBelowPlaceConfig> p_66605_) {
      super(p_66605_);
   }
    ...
    */
   public boolean place(FeaturePlaceContext<TestBelowPlaceConfig> pContext) {
      TestBelowPlaceConfig config = pContext.config();
      ...
      for(int l = 0; l < config.tries(); ++l) {
         ...
         if (config.belowPredicate().test(worldgenlevel, blockpos$mutableblockpos.below()) && biomeBelow == originBiome && config.feature().value().place(worldgenlevel, pContext.chunkGenerator(), randomsource, blockpos$mutableblockpos)) {
               ++i;
         }
      }
      return i > 0;
