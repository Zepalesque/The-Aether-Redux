...
public class FieldsprootTreeConfig implements FeatureConfiguration {
    public static final Codec<FieldsprootTreeConfig> CODEC = RecordCodecBuilder.create((mushroom) ->
            mushroom.group(BlockStateProvider.CODEC.fieldOf("leaf_provider").forGetter((config) -> config.leafProvider),
                    ...
                    .apply(mushroom, FieldsprootTreeConfig::new));
    ...
    public FieldsprootTreeConfig(BlockStateProvider leafProvider, BlockStateProvider logProvider, BlockStateProvider woodProvider, IntProvider vineLength, int patchXZSpread, int patchYSpread, int patchTries) {
