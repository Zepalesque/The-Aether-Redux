...
public class ReduxRegion extends Region {
	...
	private static ParameterPoint parameterPoint(Parameter temp, Parameter span) {
		return new Climate.ParameterPoint(temp, span, FULL, FULL, FULL, FULL, 0);
	}
	...
	public void addBiomes(
		...
		var frosted =
			ReduxBiomes
				/*ReduxConfig.COMMON.enable_snowy_biomes.get() ?*/ .FROSTED_FORESTS /*: AetherBiomes.SKYROOT_FOREST*/;
		...
		var highfields =
			ReduxBiomes
				/*ReduxConfig.COMMON.enable_skyfields.get() ?*/ .SKYFIELDS /*: AetherBiomes.SKYROOT_MEADOW*/;
		...
		var grove =
			ReduxBiomes
				/*ReduxConfig.COMMON.enable_gilded_biomes.get() ?*/ .GILDED_GROVES /*: AetherBiomes.SKYROOT_GROVE*/;
		...
		this.addBiome(mapper, parameterPoint(temp1, span(0.3F, 1.0F)), cloudcaps);

		this.addBiome(mapper, parameterPoint(temp2, span(-1f, -0.4f)), blight);
		this.addBiome(mapper, parameterPoint(temp2, span(-0.4F, -0.15F)), AetherBiomes.SKYROOT_FOREST);
		this.addBiome(mapper, parameterPoint(temp2, span(-0.15F, 0.0F)), AetherBiomes.SKYROOT_MEADOW);
		this.addBiome(mapper, parameterPoint(temp2, span(0.0F, 0.2F)), AetherBiomes.SKYROOT_WOODLAND);
		this.addBiome(
			mapper,
		...
		this.addBiome(mapper, parameterPoint(temp3, span(-0.3F, -0.1F)), AetherBiomes.SKYROOT_FOREST);
		this.addBiome(mapper, parameterPoint(temp3, span(-0.1F, 0.2F)), AetherBiomes.SKYROOT_MEADOW);
		this.addBiome(mapper, parameterPoint(temp3, span(0.2F, 0.5F)), ReduxBiomes.SKYFIELDS);
		...
		this.addBiome(mapper, parameterPoint(temp4, span(-1.0F, -0.6F)), AetherBiomes.SKYROOT_MEADOW);
		...
		this.addBiome(mapper, parameterPoint(temp5, span(-0.4F, 0.3F)), grove);
		/* addBiome(
			mapper,
