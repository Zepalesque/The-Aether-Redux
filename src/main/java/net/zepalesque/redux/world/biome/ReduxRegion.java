...
public class ReduxRegion extends Region {
    ...
    public void addBiomes(Registry<Biome> registry, Consumer<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> mapper) {
        ...
        if (ReduxConfig.COMMON.enable_snowy_biomes.get()) {
            this.addBiome(mapper, new Climate.ParameterPoint(range(-0.8D, -0.2D), range(-1.0D, -0.6D), fullRange, fullRange, fullRange, fullRange, 0L), ReduxBiomes.GLACIAL_TAIGA);
            this.addBiome(mapper, new Climate.ParameterPoint(range(-0.3D, 0.1D), range(-0.6D, 0.0D), fullRange, fullRange, fullRange, fullRange, 0L), ReduxBiomes.FROSTED_TUNDRA);
        } else {
        ...
        if (ReduxConfig.COMMON.enable_gilded_groves.get()) {
            this.addBiome(mapper, new Climate.ParameterPoint(range(0.0D, 0.4D), range(0.0D, 0.8D), fullRange, fullRange, fullRange, fullRange, 0L), ReduxBiomes.GILDED_GROVES);
        } else {
        ...
        if (ReduxConfig.COMMON.enable_cloudcap_jungle.get()) {
            this.addBiome(mapper, new Climate.ParameterPoint(range(-0.2D, 0.6D), range(0.8D, 1.0D), fullRange, fullRange, range(0.25D, 1.25D), fullRange, 0L), ReduxBiomes.CLOUDCAP_JUNGLE);
        } else {
