...
public class ReduxRegion extends Region {
    ...
    public void addBiomes(Registry<Biome> registry, Consumer<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> mapper) {
        ...
        if (ReduxConfig.COMMON.enable_snowy_biomes.get()) {
            this.addBiome(mapper, new Climate.ParameterPoint(range(-0.8D, -0.1D), range(-1.0D, -0.6D), fullRange, fullRange, fullRange, fullRange, 0L), ReduxBiomes.GLACIAL_TAIGA);
            this.addBiome(mapper, new Climate.ParameterPoint(range(-0.2D, 0D), range(-0.6D, 0.0D), fullRange, fullRange, fullRange, fullRange, 0L), ReduxBiomes.FROSTED_TUNDRA);
        } else {
