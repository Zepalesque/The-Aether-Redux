...
public class BlightwillowRootsPlacer extends RootPlacer {
	public static final MapCodec<BlightwillowRootsPlacer> CODEC = RecordCodecBuilder.mapCodec(
		(builder) ->
			builder
				.group(
					IntProvider.CODEC.fieldOf("trunk_offset_y").forGetter(
						(instance) -> instance.trunkOffsetY
					),
					Codec.INT.optionalFieldOf("max_root_depth", 2).forGetter(
						(instance) -> instance.maxRootDepth
					),
	...
	public BlightwillowRootsPlacer(
		IntProvider trunkOffset,
		int maxRootDepth,
	...
	private static final Direction[] HORIZONTAL_PLANE = Direction.Plane.HORIZONTAL.stream().toArray(
		Direction[]::new
	);
	...
	public boolean placeRoots(
		...
		if (level.isStateAtPosition(origin.below(), (state) -> !isDirt(state))) return false;
		...
		for (var dir : Direction.Plane.HORIZONTAL) {
			...
			for (var i = -1; i > -2 - this.maxRootDepth; i--) {
				...
				if (this.validRootPos(level, testPos)) if (i < -this.maxRootDepth) {
					this.unshuffle();
					return false;
				} else continue;
				min = i + 1;
		...
		if (this.validateAll(level, this.placements)) {
			this.placements.forEach((pos, useWood) ->
				setter.accept(
					pos,
					!useWood || !ReduxConfig.SERVER.use_wood_blocks.get()
						? treeConfig.trunkProvider.getState(random, pos)
						: this.wood.getState(random, pos)
				)
			);
	...
	private boolean validateAll(LevelSimulatedReader level, Map<BlockPos, Boolean> placements) {
		for (var key : placements.keySet()) if (!this.validRootPos(level, key)) return false;
		return true;
	}
	...
	protected boolean validRootPos(LevelSimulatedReader level, BlockPos pos) {
		return level.isStateAtPosition(
			pos,
			(state) ->
				state.isAir() || state.is(BlockTags.REPLACEABLE) || state.is(BlockTags.REPLACEABLE_BY_TREES)
		);
	}
