...
public class PatchTreeDecorator extends TreeDecorator {
	public static final MapCodec<PatchTreeDecorator> CODEC = RecordCodecBuilder.mapCodec((builder) ->
		builder
			.group(
	...
	public void place(Context ctx) {
		...
		if (!logs.isEmpty()) {
			...
			logs
				...
				.forEach((pos) -> this.place(this.xzSpread, this.ySpread, this.tries, pos, ctx));
		}
	}
