...
public class BranchLeavesDecorator extends TreeDecorator {
	public static final MapCodec<BranchLeavesDecorator> CODEC = RecordCodecBuilder.mapCodec(
		(builder) ->
			builder
				.group(
	...
	public void place(Context context) {
		...
		for (var pos : context.logs()) if (pos.getY() > yTest) yTest = pos.getY();
		var highest = yTest;
		var positions = context
			...
			.filter(
				(pos) ->
					pos.getY() > highest - range &&
					(context.random().nextBoolean()
						? Mth.isMultipleOf(pos.getY(), 2)
						: Mth.isMultipleOf(pos.getY() + 1, 2))
			)
			.toList();
