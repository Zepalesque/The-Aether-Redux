...
public class VineDecorator extends TreeDecorator {
	public static final MapCodec<VineDecorator> CODEC = RecordCodecBuilder.mapCodec((builder) ->
		builder
			.group(
				Codec.floatRange(0.0F, 1.0F)
					.fieldOf("probability")
					.forGetter((instance) -> instance.probability),
				BlockStateProvider.CODEC.fieldOf("plant_body_provider").forGetter(
					(instance) -> instance.bodyBlock
				),
				BlockStateProvider.CODEC.fieldOf("plant_head_provider").forGetter(
					(instance) -> instance.headBlock
				),
				IntProvider.codec(1, 128)
					.fieldOf("length")
					.forGetter((instance) -> instance.length),
				BlockPredicate.CODEC.optionalFieldOf("predicate").forGetter(
	...
	public VineDecorator(
		float vineProbability,
		BlockStateProvider bodyBlock,
		BlockStateProvider headBlock,
	...
	public void place(TreeDecorator.Context context) {
		...
		} else Redux.LOGGER.warn(
			"Failed BlockPredicate check as level was not an instance of WorldGenLevel!"
		);
		...
		for (var leafPos : leavesClone) {
			...
			try {
				...
				else if (y < LOWEST_BY_XZ.get(x, z)) LOWEST_BY_XZ.put(x, z, y);
