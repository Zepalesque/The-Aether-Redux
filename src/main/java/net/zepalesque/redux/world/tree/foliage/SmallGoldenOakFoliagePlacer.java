...
public class SmallGoldenOakFoliagePlacer extends FoliagePlacer {
	public static final MapCodec<SmallGoldenOakFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(
		builder -> foliagePlacerParts(builder).apply(
			builder,
			SmallGoldenOakFoliagePlacer::new
		)
	);
	...
	protected boolean shouldSkipLocation(
		...
		if (y == 0 || y == -4) return x + z >= radius * 2 && rand.nextFloat() < 0.75F;
		else if (y == -1 || y == -3) {
		...
		} else return x + z >= radius * 2 && rand.nextFloat() < 0.25F;
	}
