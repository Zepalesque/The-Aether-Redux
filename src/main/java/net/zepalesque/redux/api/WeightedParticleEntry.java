...
public record WeightedParticleEntry(ParticleOptions particle, Either<Integer, Float> probability) {
	...
	private static final Codec<ParticleOptions> SIMPLE_PARTICLE_CODEC =
		BuiltInRegistries.PARTICLE_TYPE.byNameCodec().flatXmap(
			WeightedParticleEntry::simple,
			(options) -> DataResult.success(options).map(ParticleOptions::getType)
		);
	...
	public static final Codec<WeightedParticleEntry> CODEC = RecordCodecBuilder.create((builder) ->
		builder
			.group(
	...
	public static DataResult<ParticleOptions> simple(ParticleType<?> type) {
		return type instanceof ParticleOptions simple
			? DataResult.success(simple)
			: DataResult.error(() ->
					"Particle type %s does not implement ParticleOptions!".formatted(
