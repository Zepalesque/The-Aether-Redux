...
public class ReduxMapData extends ReduxDataMapProvider {
	...
	protected void gather(HolderLookup.Provider provider) {
		...
		biomeTints.add(ReduxBiomes.SKYFIELDS, ReduxColors.Tints.SKYFIELDS_GRASS_COLOR, false);
		biomeTints.add(ReduxBiomes.CLOUDCAP_RIDGE, ReduxColors.Tints.CLOUDCAP_GRASS_COLOR, false);
		...
		this.addCompost(compostables, ReduxBlocks.VERBENA_CLUSTER, 0.3F);
		...
		this.addLeafParticle(
			...
			ReduxParticles.BLIGHTWILLOW_LEAF,
			20
		);
		this.addLeafParticle(
			leaves,
			ReduxBlocks.INFECTED_BLIGHTWILLOW_LEAVES,
			ReduxParticles.INFECTED_BLIGHTWILLOW_LEAF,
			25
		);
		...
		statelistModifs.add(
			UnityStateLists.FLUTEMOSS.getKey(),
			HolderSet.direct(entries.getOrThrow(ReduxStateListEntries.ECHYSIA)),
			false
		);
	}
}
