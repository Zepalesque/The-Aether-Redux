...
public class ReduxParticleData extends ReduxParticleProvider {
	...
	protected void addDescriptions() {
		...
		this.spriteSet(ReduxParticles.MOONFIR_LEAF.get(), Redux.loc("leaves/moonfir"));
		this.spriteSet(ReduxParticles.BLIGHTWILLOW_LEAF.get(), Redux.loc("leaves/blightwillow"));
		this.spriteSet(
			ReduxParticles.INFECTED_BLIGHTWILLOW_LEAF.get(),
			Redux.loc("leaves/infected_blightwillow")
		);
		...
		var lightningTextures = ArrayUtil.generateContents(new ResourceLocation[10], (i) ->
			Redux.loc("lightning/lightning" + i)
		);
		this.spriteSet(ReduxParticles.WHIRLWIND_LIGHTNING.get(), List.of(lightningTextures));

		ResourceLocation[] sparkTextures = ArrayUtil.generateContents(new ResourceLocation[3], (i) ->
			Redux.loc("spark/spark" + i)
		);
