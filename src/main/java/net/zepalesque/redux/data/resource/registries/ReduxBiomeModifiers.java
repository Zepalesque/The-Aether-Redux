...
public class ReduxBiomeModifiers {
	...
	public static void bootstrap(BootstrapContext<BiomeModifier> context) {
		...
		context.register(
			ADD_CLOUDBED,
			new ConditionalBiomeModifier(
				Holder.direct(cloudbed),
		...
		context.register(
			...
			new ConditionalBiomeModifier(
				Holder.direct(lakes),
				conditions.getOrThrow(ReduxConditions.LAKES)
			)
		);

		var catfish = new BiomeModifiers.AddSpawnsBiomeModifier(
			biomes.getOrThrow(ReduxTags.Biomes.HAS_CAT_FISH),
			List.of(new MobSpawnSettings.SpawnerData(ReduxEntities.CAT_FISH.get(), 5, 1, 5))
		);
		context.register(
			ADD_CAT_FISH,
			new ConditionalBiomeModifier(
				Holder.direct(catfish),
				conditions.getOrThrow(ReduxConditions.LAKES)
			)
		);
		...
		var sky = new SkiesModifier(
			Optional.of(
				new SkiesModifier.DefaultSkySettings(
					biomes.getOrThrow(ReduxTags.Biomes.MODIFY_SKY_COLOR),
					Optional.of(0x9FA4DD),
					Optional.of(0xBEC4E5)
				)
			),
			ImmutableMap
				.<Holder<Biome>, Integer>builder() // sky
		...
		context.register(
			...
			new ConditionalBiomeModifier(
				Holder.direct(sky),
				conditions.get(ReduxConditions.SKY_COLORS).orElseThrow()
			)
		);

		var water = new WaterModifier(
			Optional.of(
				new WaterModifier.DefaultWaterSettings(
					biomes.getOrThrow(ReduxTags.Biomes.MODIFY_WATER_COLOR),
					Optional.of(0x85BDD1),
					Optional.of(0x182226)
				)
			),
			ImmutableMap
				.<Holder<Biome>, Integer>builder() // water
		...
		context.register(
			...
			new ConditionalBiomeModifier(
				Holder.direct(water),
				conditions.get(ReduxConditions.WATER_COLORS).orElseThrow()
			)
		);
		...
		context.register(
			MUSIC_MODIFY,
			new MusicModifier(
				biomes.getOrThrow(ReduxTags.Biomes.MODIFY_MUSIC),
				new MusicModifier.MusicOperator(
					Optional.empty(),
					Optional.of(ReduxMusic.MUSIC_MIN),
					Optional.of(ReduxMusic.MUSIC_MAX),
					Optional.empty()
				),
				Optional.empty()
			)
		...
		context.register(
			...
			new BiomeModifiers.AddCarversBiomeModifier(
				...
				HolderSet.direct(carvers.getOrThrow(ReduxCarverConfig.AETHER_CAVES)),
