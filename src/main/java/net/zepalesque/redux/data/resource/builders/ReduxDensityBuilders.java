...
public class ReduxDensityBuilders {
	...
	protected static DensityFunction getFunction(
	...
	public static NoiseGeneratorSettings reduxSkylandsNoiseSettings(
		HolderGetter<DensityFunction> functions,
		HolderGetter<NormalNoise.NoiseParameters> noise
	) {
		BlockState holystone = AetherBlocks.HOLYSTONE.get()
			.defaultBlockState()
		...
		return new NoiseGeneratorSettings(
			...
			AetherNoiseBuildersMixin.callCreateNoiseRouter(
				functions,
				noise,
				getFunction(functions, ReduxDensityFunctions.REDUX_FINAL_DENSITY)
			), // noiseRouter
	...
	protected static ResourceKey<DensityFunction> copyKey(
	...
	public static DensityFunction get(
		HolderGetter<DensityFunction> densityFunctions,
		ResourceKey<DensityFunction> key
	) {
		return new DensityFunctions.HolderHolder(densityFunctions.getOrThrow(key));
	...
	public static DensityFunction get(RegistryAccess access, ResourceKey<DensityFunction> key) {
		Optional<HolderLookup.RegistryLookup<DensityFunction>> optional = access.lookup(
