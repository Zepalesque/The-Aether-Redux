	...
	public static void bootstrap(BootstrapContext<DensityFunction> context) {
		...
		context.register(
			...
			DensityFunctions.mul(
				new PerlinNoiseFunction(noises.getOrThrow(ReduxNoises.CLOUDBED_NOISE), 0.01D, 0.0D, 42),
		...
		context.register(
			CLOUDBED_Y_OFFSET,
			DensityFunctions.mul(
				new PerlinNoiseFunction(noises.getOrThrow(ReduxNoises.CLOUDBED_Y_OFFSET), 0.005D, 0.0D, 95),
				DensityFunctions.constant(1.5D)
			)
		);
