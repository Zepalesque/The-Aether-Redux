...
public class ReduxConditions {
	...
	public static void bootstrap(BootstrapContext<Condition<?>> context) {
		...
		context.register(
			SKY_COLORS,
			new ConfigCondition(ReduxConfig.SERVER.serializerID(), ReduxConfig.SERVER.redux_sky_colors)
		);
		context.register(
			CLOUDBED,
			new ConfigCondition(ReduxConfig.SERVER.serializerID(), ReduxConfig.SERVER.cloudbed)
		);
		context.register(
			LAKES,
			new ConfigCondition(ReduxConfig.SERVER.serializerID(), ReduxConfig.SERVER.lakes)
		);
		context.register(
			WATER_COLORS,
			new ConfigCondition(ReduxConfig.SERVER.serializerID(), ReduxConfig.SERVER.redux_water_colors)
		);
		context.register(
			MOSSY_ORE,
			new ConfigCondition(ReduxConfig.SERVER.serializerID(), ReduxConfig.SERVER.mossy_holystone_gen)
		);
	}
