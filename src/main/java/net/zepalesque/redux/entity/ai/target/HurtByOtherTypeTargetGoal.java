...
public class HurtByOtherTypeTargetGoal extends TargetGoal {
	...
	public boolean canUse() {
		var i = this.mob.getLastHurtByMobTimestamp();
		var livingentity = this.mob.getLastHurtByMob();
		if (i != this.timestamp && livingentity != null) {
			if (livingentity.getType() == EntityType.PLAYER && this.mob.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
				return false;
			} else {
				for(var oclass : this.toIgnoreDamage) {
	...
	protected void alertOthers() {
		var d0 = this.getFollowDistance();
		var aabb = AABB.unitCubeFromLowerCorner(this.mob.position()).inflate(d0, 10.0D, d0);
		var list = this.mob.level().getEntitiesOfClass(this.mob.getClass(), aabb, EntitySelector.NO_SPECTATORS);
