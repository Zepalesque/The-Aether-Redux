...
public class Ember extends Projectile {
	...
	public void tick() {
		...
		if (
			hitresult.getType() != HitResult.Type.MISS && !EventHooks.onProjectileImpact(this, hitresult)
		) this.onHit(hitresult);

		if (hitresult.getType() == HitResult.Type.MISS) {
			if (!this.isNoGravity()) this.setDeltaMovement(
	...
	protected void readAdditionalSaveData(CompoundTag compound) {
		...
		if (
			compound.contains("Hits") && compound.get("Hits") instanceof ListTag hits
		) for (Tag tag : hits)
			if (tag.getType() == IntArrayTag.TYPE) this.hitEntities.add(NbtUtils.loadUUID(tag));
	}
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		if (
			...
			!this.ownedBy(livingentity) &&
			!this.originatedFrom(livingentity) &&
			!this.hasHit(livingentity) &&
	...
	protected void onHitBlock(BlockHitResult result) {
		...
		for (int i = 0; i < Mth.floor(velocity.length() * 15); i++) {
			...
			switch (axis) {
				case X -> this.level().addParticle(
					ReduxParticles.SPARK.get(),
					loc.x(),
					loc.y(),
					loc.z(),
					velocity.length(),
		...
		SoundEvent sound =
			velocity.length() <= 0.75
				? ReduxSounds.EMBER_BOUNCE_SMALL.get()
				: velocity.length() <= 1.5
					? ReduxSounds.EMBER_BOUNCE_MEDIUM.get()
