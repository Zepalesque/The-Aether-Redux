...
public class VeridiumDart extends AbstractDart {
	...
	public VeridiumDart(
		EntityType<? extends VeridiumDart> entityType,
		Level level,
		double x,
		double y,
		double z,
	...
	public void tick() {
		super.tick();
		if (this.level().isClientSide() && !this.inGround) this.level().addParticle(
	...
	public static class Uninfused extends AbstractDart {
		...
		public Uninfused(
			EntityType<? extends VeridiumDart.Uninfused> entityType,
			Level level,
			double x,
			double y,
			double z,
