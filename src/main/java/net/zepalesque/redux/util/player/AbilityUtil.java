...
public class AbilityUtil {
    ...
    public static void doDoubleJumpMovement(LivingEntity entity) {
        ...
        spawnDoubleJumpParticles(entity.getLevel(), entity.position().x, entity.position().y, entity.position().z, 1.5D, 12);
        entity.getLevel().playSound(null, entity.position().x, entity.position().y, entity.position().z, ReduxSoundEvents.BOOST_JUMP.get(), SoundSource.PLAYERS, 0.4f,
                0.9F + entity.getLevel().random.nextFloat() * 0.2F);
    }
    ...
    public static void shootFireballs(LivingEntity entity) {
        if (EquipmentUtil.hasCurio(entity, ReduxItems.SOLAR_EMBLEM.get())) {
            if (EquipmentUtil.getCurios(entity, ReduxItems.SOLAR_EMBLEM.get()).size() == 1) {
                ...
                VolatileFireCrystal crystal = new VolatileFireCrystal(entity.getLevel(), entity);
                ...
                if (!entity.getLevel().isClientSide) {
                    entity.getLevel().addFreshEntity(crystal);
                    entity.getLevel().playSound(null, entity.position().x, entity.position().y, entity.position().z, ReduxSoundEvents.FIREBALL_SHOOT.get(), SoundSource.PLAYERS, 0.4f,
                            0.9F + entity.getLevel().random.nextFloat() * 0.2F);
                ...
                VolatileFireCrystal crystal1 = new VolatileFireCrystal(entity.getLevel(), entity);
                ...
                if (!entity.getLevel().isClientSide) {
                    entity.getLevel().addFreshEntity(crystal1);
                    entity.getLevel().playSound(null, entity.position().x, entity.position().y, entity.position().z, ReduxSoundEvents.FIREBALL_SHOOT.get(), SoundSource.PLAYERS, 0.4f,
                            0.9F + entity.getLevel().random.nextFloat() * 0.2F);
