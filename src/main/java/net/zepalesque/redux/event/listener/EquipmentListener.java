...
public class EquipmentListener {
    ...
    {
        if (event.getEntity() instanceof ServerPlayer player && !player.getLevel().isClientSide())
        {
    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (!target.getLevel().isClientSide() && EquipmentUtil.hasCurio(target, ReduxItems.SHROOM_RING.get()) && (target.getHealth() / target.getMaxHealth()) <= 0.66666667F) {
                if (target.hasEffect(ReduxEffects.ADRENALINE_RUSH.get()) && !target.hasEffect(ReduxEffects.ADRENAL_FATIGUE.get())) {
                    ...
                    if (target.getLevel().getRandom().nextFloat() <= delta) {
                    ...
                    if (target.getLevel().getRandom().nextFloat() <= delta && !target.hasEffect(ReduxEffects.ADRENAL_FATIGUE.get())) {
        ...
        if (target != null && event.getSource().getDirectEntity() instanceof Player player && !event.getSource().getMsgId().equals("aether_redux.ember") && event.getAmount() > 1.0) {
            if (EquipmentUtil.hasCurio(player, ReduxItems.SENTRY_RING.get())) {
                ...
                RandomSource source = target.getLevel().getRandom();
                ...
                for (int i = 1; i <= embers; i++) {
                    ...
                    Ember ember = new Ember(target.getLevel(), player, target);
                    ...
                    if (!player.getLevel().isClientSide()) {
                        player.getLevel().addFreshEntity(ember);
                    }
                }
    ...
    public static void tickPlayer(LivingEvent.LivingTickEvent event)
    {
        ...
        if (EquipmentUtil.hasCurio(entity, ReduxItems.GRAND_VICTORY_MEDAL.get())) {
            ...
            if (i > 0 && entity.tickCount % i == 0 && entity.getLevel().getRandom().nextBoolean())
            {
                entity.heal(1);
            }
        }
