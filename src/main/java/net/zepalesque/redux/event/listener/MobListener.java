...
public class MobListener {
    ...
    public static void livingUpdate(LivingEvent.LivingTickEvent event) {
        ...
        if ((d0 >= (double)0.003F || d1 >= (double)0.003F) && entity instanceof Player player && !player.getLevel().isClientSide()) {
            Optional<ResourceKey<Biome>> b = player.getLevel().getBiome(player.blockPosition()).unwrapKey();
    ...
    public static void attack(LivingHurtEvent event) {
        if (event.getSource().getEntity() instanceof LivingEntity entity && EquipmentUtil.hasCurio(entity, ReduxItems.VAMPIRE_AMULET.get())) {
            ...
            {
                if (vampireAmulet.canUseAbility() && !vampireAmulet.getMob().getLevel().isDay()) {
        ...
        {
            if (event.getEntity().getLevel().isDay()) {
