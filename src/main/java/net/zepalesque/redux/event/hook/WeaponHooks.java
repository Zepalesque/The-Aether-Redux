                ...
                if (subzeroArrow.isSubzeroArrow()) {
                    for (int i = 0; i < 10; i++) {
                        if (!subzeroArrow.hitGround()) {
                            abstractArrow.getLevel().addParticle(ReduxParticleTypes.ICE_SHARD.get(), abstractArrow.getX(), abstractArrow.getY(), abstractArrow.getZ(), 0.0D, 0.0D, 0.0D);
                        }
                    }
                }
            });
            if (!abstractArrow.getLevel().isClientSide) {
